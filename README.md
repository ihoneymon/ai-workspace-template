...
PR 생성 시 브랜치 접두사에 따라 다음 Label을 자동으로 지정합니다.

| Label | 브랜치 접두사 | 용도 |
|-------|---------------|------|
| `신규기능` | `feature/`, `feat/` | 새로운 기능이나 스킬 추가 |
| `기능변경/개선` | `improve/`, `refactor/`, `chore/`, `docs/`, `codex/` | 기존 기능, 구조, 문서 및 작업 환경 개선 |
| `출시` | `release/`, `deploy/` | 버전 출시 및 배포 준비 |
| `오류수정` | `fix/`, `bugfix/`, `hotfix/` | 결함 및 회귀 수정 |
