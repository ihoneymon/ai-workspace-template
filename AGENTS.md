...
Claude Code 외의 AI 에이전트(Codex, Cursor 등)가 이 워크스페이스에서 작업할 때 따르는 운영 규칙입니다.
...
도메인, 정책, 기능, 코드 변경과 관련된 요청은 바로 코드를 뒤지기 전에
`CLAUDE.md`의 "3계층 지식 구조 > 탐색 순서"를 따라 ontology → wiki → 코드 순으로 확인한다.
...
- 워크트리가 없으면 아래 명령으로 생성한다. 상세 절차는 `.claude/skills/worktree/SKILL.md`를 참고한다.
  ```bash
  # GIT_ROOT: projects/{repo}/main 의 git 루트
  git -C ${GIT_ROOT} worktree add ../worktrees/<dir-name> -b <branch-name>
  # dir-name = branch-name의 '/' → '-' 치환
  # 생성 후 워크스페이스 루트의 .claude/hooks를 심볼릭 링크로 연결한다
  ```
...
slash command(`/commit`)를 실행할 수 없는 에이전트는 아래 형식으로 직접 커밋한다.

```
[ISSUE-KEY] 제목 (40자 이내, 이슈 키 없으면 생략)
...
- 제목과 본문 사이에 빈 줄을 둔다.
- 본문은 `-` bullet으로 구체적 변경사항을 나열한다.
- 한국어로 작성한다. 식별자(클래스명, 메서드명 등)는 그대로 유지한다.
- `Co-Authored-By` 라인은 추가하지 않는다.
- 커밋 전 `git status`로 민감 파일(`.env*`, `*.key`, `*.pem`)이 포함되지 않았는지 확인한다.
...
이 문서를 따르는 에이전트는 Claude slash command나 hook을 자동 실행하지 않는다. 다만 필요한 경우 `.claude/skills/*/SKILL.md`와 `.claude/rules/*.md`를 절차 문서로 읽고, 각자 실행 가능한 명령과 파일 수정으로 옮겨 수행한다.
하위 문서에 "slash command를 사용하라" 또는 "git 명령어를 직접 실행하지 말라"는 표현이 있으면, 비-Claude 에이전트는 해당 스킬 문서의 사전 확인·안전 규칙·검증 절차를 수동으로 수행한다.

## 역할별 작업 방식

- 코드 리뷰는 에이전트가 직접 수행한다. 리뷰 요청을 받으면 버그, 회귀 위험, 누락된 테스트, 유지보수 리스크를 우선순위로 보고한다.
...
- Node.js 프로젝트는 `package.json`의 scripts와 lockfile을 확인한다. `bun.lock` 또는 `bun.lockb`가 있고 `bun`이 설치되어 있으면 `bun`을 우선 사용하고, 아니면 `npm`/`pnpm`/`yarn` 중 lockfile에 맞는 도구를 사용한다.
- 검증이 실패하면 같은 명령을 재시도하지 말고 실패 원인을 먼저 분석한 뒤 수정한다.
