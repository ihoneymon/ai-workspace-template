...
name: dev
version: 1.0.0
description: "PRD → 설계 → 구현 → 리뷰 → 커밋/PR까지 전체 개발 사이클을 에이전트 팀이 Q&A 루프로 수행. /spec으로 작성한 계획서(.dev/)가 있는 상태에서 '구현해', '시작해', '코딩해' 등으로 구현을 이어가는 경우에도 이 스킬을 사용하라. --resume 옵션으로 기존 계획서 기반 구현을 시작한다."
argument-hint: "<기능/버그 설명> [--phase requirements|design|implement|review|complete] [--hotfix] [--base <branch>] [--status] [--resume [티켓키]]"
...
- `--resume [티켓키]`: 이전 파이프라인을 재개한다. 두 가지 모드가 있다:
  - **티켓키 지정** (`/dev --resume PROJ-2850`): `.dev/{티켓키}/state.md`를 찾는다.
    - `phase: approved` → spec이 작성한 계획서. 아래 "approved 핸드오프 절차"에 따라 실행.
    - 그 외 phase → 기존 dev 파이프라인 재개. 중단된 Step부터 재개.
  - **티켓키 없음** (`/dev --resume`): `.dev/*/state.md`를 스캔한다.
    - 1개 → 자동 선택.
    - 여러 개 → AskUserQuestion으로 사용자에게 선택 요청.
    - 0개 → `.dev/state.md`를 찾는다 (기존 dev 파이프라인). 없으면 에러.
  - state.md가 없거나 `completed`이면 에러.
...
- `DOMAIN_CONTEXT`: phase-setup 0.3에서 `wiki/*/PROJECTS.md` 매칭으로 로드된 도메인 용어(glossary)와 아키텍처 정보. 매칭되지 않으면 빈 상태.
- `PROJECT_RULES`: phase-implement Step 0.1에서 `${PROJECT_ROOT}/.claude/rules/` 하위 `.md` 파일을 모두 Read하여 로드한 프로젝트 규칙. 디렉토리가 없으면 빈 상태. coder 에이전트(구현/수정)에게 전달하여 프로젝트별 코딩 규칙을 준수하게 한다.
...
- **coder (구현)**: 설계서 전체 + 코드 맵 + 프로젝트 루트 경로 + PROJECT_RULES (있으면). `--hotfix`이면 설계서 대신 PRD + 코드 맵.
- **coder (수정)**: 수정 항목 목록 + 수정 방안 + 코드 맵 + 프로젝트 루트 경로 + PROJECT_RULES (있으면)
...
- `--resume`은 티켓키를 선택적 인자로 받는다 (`/dev --resume` 또는 `/dev --resume PROJ-2850`). 작업 설명 텍스트와 함께 사용할 수 없다.

## approved 핸드오프 절차

`--resume`으로 `phase: approved` state.md를 감지했을 때의 구체적 실행 절차. spec이 작성한 계획서를 워크트리에 옮기고 구현을 시작한다.

### 0. state.md에서 변수 복원

`.dev/{티켓키}/state.md`를 Read하여 다음 변수를 복원한다:
- `base` → 베이스 브랜치
- `project-type` → `PROJECT_TYPE`
- `code-root` → `CODE_ROOT`
- `project-git` → `PROJECT_GIT`
- `context-path` → `CONTEXT_PATH` (빈 문자열이면 없음)
- `domain` → 도메인명 (빈 문자열이면 없음)

`CONTEXT_PATH`와 `domain`이 있으면 `glossary.md`, `architecture.md`를 Read하여 `DOMAIN_CONTEXT`에 저장한다.

### 1. base 브랜치 최신화

`${PROJECT_GIT} fetch origin`으로 리모트를 최신화한다. checkout은 하지 않는다 (worktree create가 `origin/{base}` 기준으로 생성).

### 2. 워크트리 생성

`worktree` 스킬의 SKILL.md를 Read하여 `create` 프로세스를 실행한다.
- `/worktree create {티켓키}` 형태로 실행한다.
- worktree 스킬이 prefix, username, base 브랜치를 질문한다.
- 생성 완료 후 `PROJECT_ROOT = worktrees/{워크트리명}/`, `GIT_PREFIX = git -C worktrees/{워크트리명}`으로 설정한다.

### 3. .dev/ 문서 복사

`.dev/{티켓키}/`의 문서를 워크트리의 `.dev/`로 복사한다:
1. `mkdir -p ${PROJECT_ROOT}/.dev`
2. `cp .dev/{티켓키}/prd.md ${PROJECT_ROOT}/.dev/prd.md`
3. `cp .dev/{티켓키}/design.md ${PROJECT_ROOT}/.dev/design.md`
4. `cp .dev/{티켓키}/codemap.md ${PROJECT_ROOT}/.dev/codemap.md`

> `.dev/{티켓키}/`가 이미 `${PROJECT_ROOT}/.dev/` 안에 있으면 복사를 건너뛴다.

### 4. state.md 갱신

`${PROJECT_ROOT}/.dev/state.md`에 Write:
```yaml
phase: implement
status: in_progress
branch: {브랜치명}
...
phase-setup Step 0.5a와 동일. `${PROJECT_ROOT}/.gitignore`에 `.dev/` + 프로젝트 타입별 빌드 아티팩트 패턴을 추가한다 (이미 존재하는 패턴은 건너뜀).

### 6. phase-implement 시작

Phase 라우팅에 따라 `phase-implement.md`를 Read하고 실행한다. 이후 phase-review → phase-complete 순서로 진행한다.

> phase-complete의 5.4(status.md 갱신)와 5.5(context 환류 제안)에서 `DOMAIN_CONTEXT`를 사용하므로, Step 0에서 반드시 복원해야 한다.
