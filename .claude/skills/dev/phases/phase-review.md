...
- 프로젝트 타입에 따른 lint 명령: kotlin-gradle → `./gradlew ktlintFormat`, node → lockfile/runtime 감지 후 `bun run lint --fix`, `pnpm run lint --fix`, `yarn lint --fix`, `npm run lint -- --fix` 중 하나, python → `ruff format .`
...
2. CLAUDE.md에 빌드 명령이 없으면 → 프로젝트 타입에서 기본값을 사용한다:
   ...
   | node | lockfile/runtime 감지 후 `bun run build`, `pnpm run build`, `yarn build`, `npm run build` 중 하나 (package.json의 scripts.build가 있을 때만) |
...
- `CONTEXT_FILE: ${PROJECT_ROOT}/.dev/context.md` (있으면)
...
if findings에 Critical(QA) 또는 CRITICAL(ZT)이 있으면:
    해당 항목만 사용자에게 표시
    coder로 자동 수정 (수정 모드: 항목 목록 + 수정 방안 + 코드 맵 + PROJECT_ROOT + PROJECT_RULES)
    did_fix = true
...
if did_fix:
    ...
    if Warning/Info(QA) 또는 HIGH/MEDIUM(ZT) 항목이 있으면:
        ...
        if 수정 선택:
            coder로 수정 (PROJECT_RULES 포함) → qa-manager 1회 확인 리뷰 → phase-complete
