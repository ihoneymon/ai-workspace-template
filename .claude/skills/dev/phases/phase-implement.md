...
**Step 0.1**: 프로젝트 규칙 로드.
- `${PROJECT_ROOT}/.claude/rules/` 디렉토리 존재를 확인한다 (Glob 또는 Bash `test -d`).
- **존재하면**: 하위 `.md` 파일을 모두 Read하여 `PROJECT_RULES` 변수에 저장한다.
- **존재하지 않으면**: `PROJECT_RULES`를 빈 상태로 두고 건너뛴다.

`Task(subagent_type="coder")` — prompt에 다음을 포함:
- `CONTEXT_FILE: ${PROJECT_ROOT}/.dev/context.md` (프로젝트 타입, 구조, 코드 맵 포함)
- 확정된 설계서 (Step 0에서 로드한 설계 문서 전체)
- 프로젝트 루트 경로 (작업 경로 기준 참조)
- 프로젝트 규칙 (`PROJECT_RULES`, 있으면). "다음은 프로젝트의 구현 규칙이다. 반드시 준수할 것." 안내와 함께 전달.
