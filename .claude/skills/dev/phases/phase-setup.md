...
4. **도메인 컨텍스트 탐색**: 현재 레포와 매칭되는 도메인 컨텍스트를 찾는다.
   ...
   - `wiki/*/PROJECTS.md`를 Grep하여 해당 레포를 참조하는 도메인을 찾는다.
   - 매칭되면 해당 도메인의 `glossary.md`, `architecture.md`를 Read하여 `DOMAIN_CONTEXT`에 저장한다.
   - 매칭되지 않으면 `DOMAIN_CONTEXT`는 빈 상태로 진행한다.
     사용자에게 안내: "현재 레포와 매칭되는 도메인 컨텍스트가 없습니다. `/new-domain`으로 등록하면 이후 작업에서 용어/아키텍처를 참조할 수 있습니다."
...
## 0.5 작업환경 생성
격리된 작업환경을 생성한다.
- ARGS[0]에서 브랜치명을 생성한다. `.claude/config.json`의 `branchNaming`을 참조한다:
  1. 이슈 키 추출 시도: 대문자 영문 + `-` + 숫자 패턴 (e.g., `JIRA-123`, `PAY-456`)
  2. **이슈 키가 있으면**: `branchNaming.pattern`에 따라 브랜치명을 생성한다.
     - `{issueKey}`: 추출한 이슈 키로 치환
     - `{username}`: `branchNaming.usernameFrom`에 따라 런타임에 추출하여 치환
       - `git-email-local`: `git config user.email`에서 `@` 앞부분 (e.g., `username@mycompany.com` → `username`)
       - `git-user-name`: `git config user.name` 그대로
       - git config 값이 비어있으면 AskUserQuestion으로 username을 직접 입력받는다.
     - 예: pattern `feature/{username}/{issueKey}` → 브랜치 `feature/username/PROJ-3060`
     - **워크트리 디렉토리명**: 브랜치명의 `/`를 `-`로 치환하여 사용한다 (e.g., `feature-username-PROJ-3060`). 이는 중첩 디렉토리 생성을 방지한다.
     - `branchNaming` 설정이 없으면 이슈 키를 그대로 브랜치명으로 사용한다 (기존 동작).
  3. **이슈 키가 없으면**: 핵심 키워드 추출, 한국어→영어 번역, 최대 40자 (e.g., `로그인 기능 추가` → `login-feature`). `branchNaming.pattern`의 `{issueKey}` 자리에 키워드를 넣는다.
- **워크스페이스 모드** (`main/.git` 존재): `worktree` 스킬을 Read하여 `create <branch-name>` 프로세스를 실행한다.
  - 실행 후 `test -d worktrees/<dir-name>`으로 디렉토리 존재를 확인한다 (`<dir-name>` = 브랜치명의 `/`→`-` 치환).
  - 실패 시: "워크트리 생성에 실패했습니다. `worktrees/<dir-name>`이 존재하지 않습니다." → 사용자에게 재시도/중단 선택지를 제시한다. `GIT_PREFIX`는 갱신하지 않는다.
...
0.3~0.4에서 수집한 정보를 `${PROJECT_ROOT}/.dev/context.md`에 통합하여 Write한다.
이 파일은 이후 모든 agent Task에 `CONTEXT_FILE` 경로로 전달된다.
...
## 프로젝트
- 타입: <PROJECT_TYPE> (kotlin-gradle | node | python | 기타)
- 빌드/테스트 명령: (예: `./gradlew test`, `bun test`)

## 디렉토리 구조 (최상위 2레벨)
<0.3에서 수집한 구조>

## 코딩 컨벤션
<0.3에서 읽은 CLAUDE.md 내용. 없으면 "CLAUDE.md 없음">

## 도메인 컨텍스트
<0.3에서 수집한 DOMAIN_CONTEXT. 없으면 "도메인 컨텍스트 없음">

## 코드 맵
<0.4에서 작성한 코드 맵 전체>
```

워크스페이스 정적 컨텍스트(`.claude/context/shared.md`)는 각 agent가 직접 로드하므로 이 파일에 복사하지 않는다.
