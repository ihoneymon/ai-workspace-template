...
```bash
# JSON으로 열고 surface_ref를 추출 (스크립트 권장 — --json은 전역 플래그라 browser 앞에 온다)
...
`cmux browser identify`는 터미널에서 호출하면 `Error: invalid_params: Surface is not a browser`를 내고, surface를 줘도 `OK`만 반환해 탐색에 쓸모없다. **대신 `tree`를 파싱하라** — 읽기 전용이며 포커스/선택을 바꾸지 않는다.

```bash
cmux tree --all --json | jq -r '
...
원하는 surface의 `ref`를 골라 `--surface`로 대상 지정한다. raw tree에는 URL·제목이 담기니 로그/붙여넣기 전에 필요하면 가려라.
...
cmux tree --all --json | jq -r '
  .windows[]?.workspaces[]?.panes[]?.surfaces[]?
  | select(.type=="browser") | .ref'   # 목록에서 사라졌으면 정리 완료
```
