...
```json
{
  ...
  "projects": {
    "required": [
      { "repo": "org/repo-api", "desc": "백엔드 API 서버" }
    ],
    "optional": [
      { "repo": "org/repo-web", "desc": "프론트엔드 웹앱" }
    ]
  }
}
```
...
| agent-browser | `which agent-browser` | `npm install -g agent-browser` 실행 |

순서가 중요하다: brew → git → gh → jq → node → agent-browser. brew가 있어야 나머지를 설치할 수 있고, node가 있어야 npm이 사용 가능하다.
